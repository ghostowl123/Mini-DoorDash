...
-- delete cascade -> 级联删除 关联键删除 这个表里的关联值也会被删除
-- soft delete Vs physical delete -> soft delete not really delete statement adding a boolean variable hide it
-- physical delete -> real delete from the database
-- order item 中 price不是多余项 处理客服免单或者调整价格给单独一单、
-- Serial key word for auto increment also handle the multi thread高并发场景 sync
